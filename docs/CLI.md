...
| `config validate` | offline (no broker) | any | Parse + fully validate a serve config (from the same flags / env / `--config` file `serve` reads) and report ok/errors WITHOUT starting the broker (#637). Exit 0 valid, frozen nonzero usage code invalid. Same validator as `serve --check-only`. |
...
| `--consumer-credit <n>` | u32 | `2048` (`DEFAULT_CONSUMER_CREDIT`, aliased to `ironbus_server::engine::DEFAULT_CONSUMER_CREDIT`, the Kafka-class auto-tune CEILING) | messages | The per-CONNECTION un-acked credit CEILING (#552). The window AUTO-TUNES: it starts at a 64-message floor and grows toward this ceiling as the consumer keeps draining (a TCP-congestion-style AIMD), so a fast/loopback consumer is not pinned at 64/RTT, then backs off under backpressure. A fetch delivers `min(requested, current credit window, byte budget, group window)`, so one stuck consumer cannot drain a peer's budget. This ceiling is also the worst-case in-flight count the refuse-to-boot RAM guard charges. Must be at least 1; a value below 64 caps the auto-tune below the floor. |
...
| `--drain-timeout-ms <n>` | u64 | `30000` (30 s, `DEFAULT_DRAIN_TIMEOUT_MS`); `0` = wait indefinitely | milliseconds | The graceful-shutdown DRAIN TIMEOUT (#637). On `SIGTERM`/`SIGINT` the broker FIRST flips `/readyz` to 503 (so an orchestrator stops routing new work to it), THEN drains in-flight work (flush the pending produce batch + checkpoint every group, the #195 path), and force-exits if the drain has not finished within this window — so a k8s-style `terminationGracePeriod` never hangs. A force-exit loses NO acked data (every ack was fsynced before its ack, I2); at most the final cursor checkpoint is skipped, so a restart may redeliver up to `--checkpoint-interval` of already-acked messages (the at-least-once contract). `0` waits indefinitely (the historical unbounded drain). |
| `--audit-log <stderr\|@path>` | string | off (no audit stream) | `stderr` or `@<path>` | The security AUDIT-EVENT stream sink (#635): where structured `authn_outcome` / `authz_denial` / `secret_permission_refusal` / `config_change` events are written. `stderr` writes to the broker's stderr log stream; `@<path>` appends to an owner-only (`0600`) audit file. Each event carries the identity NAME and the mechanism/scope/verb tags only — NEVER a credential (the emitter has no field for one). Off by default (a single relaxed counter bump, no formatting, no IO). A reference, never an inline secret. |
| `--check-only` | flag (no value) | off | n/a | VALIDATE the serve config (the same checks `serve` runs at startup: the storage/cluster/geo interplay, the data-dir rule, the tuning ranges, and on Unix the fail-closed bind invariant + secret-file `StrictModes`) and exit WITHOUT binding, locking, or opening anything (#637). Exit 0 on valid (`ok: serve config is valid`), the frozen nonzero usage code on invalid. The in-process twin of `config validate`. |
...
| `--autopipe` | flag (no value) | off | n/a | AUTO-PIPELINING DURABLE producer (`bench --mode publish`, #508): drive the client's `pipelined_producer_with_window` (sized by `--pubwindow`, or `DEFAULT_PIPELINE_WINDOW` = 64 when `--pubwindow` is left at its default of 1) instead of the awaited per-publish `produce`. The handle buffers a window of at-least-once publishes and flushes them as ONE group-committed batch, so a SINGLE producer keeps the window in flight and the broker collapses it under one `fdatasync` — the single-producer durable-throughput lever the awaited path cannot reach (it has one publish in flight at a time). Every publish stays durable (ack-implies-durable); only WHEN the ack is observed moves. Publish-only, mutually exclusive with `--stream` and `--fire-and-forget`. Reported additively as `auto_pipeline` in the `--json` output. |
