...
| Title | Status | Owning issue | Summary |
| --- | --- | --- | --- |
| Graceful-shutdown signal handling via `ctrlc` | Accepted | [#195](https://github.com/ELares/IronBus/issues/195) (CLOSED) | SIGINT/SIGTERM/SIGHUP checkpoint every group's cursor before exit, using the `ctrlc` crate (chosen over a hand-rolled `unsafe` sigaction Miri cannot exercise); it pulls `nix`/`libc`, allowlisted in `deny.toml`. The decision is recorded in `crates/ironbus-cli/Cargo.toml`. |
| Per-consumer in-flight credit default 64 | Accepted (file pending), issue OPEN | [#65](https://github.com/ELares/IronBus/issues/65) (OPEN) | Each connection holds at most `consumer_credit` un-acked messages (`DEFAULT_CONSUMER_CREDIT` = 64, floored to 1, no unbounded opt-out); effective bound is the min of this and the per-group window. The message-count half is implemented (#274); the per-consumer BYTE budget and wire-negotiated credit are deferred (#275), so #65 stays open. |
