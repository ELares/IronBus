...
| [0004](0004-tls-crypto-provider.md) | TLS 1.3 crypto provider — scoped aws-lc-rs allowance | **Accepted** (2026-07-06) | [#766](https://github.com/ELares/IronBus/issues/766), [#957](https://github.com/ELares/IronBus/issues/957), [#107](https://github.com/ELares/IronBus/issues/107) | Ships TLS behind a non-default `tls` feature whose crypto provider is `aws-lc-rs`, added to `deny.toml` as a documented feature-scoped C-FFI exception exactly like `zstd-sys` (ADR-0003) — default/edge-min builds stay pure-Rust. Owner ratified Option A on 2026-07-06; `deny.toml` acquires the `aws-lc-sys` allowance in the same change that adds the dependency. Implementation sequenced on #766. |
