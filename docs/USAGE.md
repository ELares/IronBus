...
| `--consumer-credit <n>` | `2048` | The per-connection un-acked message credit CEILING (#552): the most messages one connection may EVER hold un-acked. The window AUTO-TUNES from a 64 floor toward this ceiling as the consumer keeps draining (so a fast consumer is not pinned at 64/RTT) and backs off under backpressure. The effective per-fetch credit is the smaller of the current auto-tuned window, the byte budget, and the per-group `--max-in-flight` window. Must be at least 1. |
...
| `--storage <disk\|memory>` | `disk` | The storage backend. `disk` is the durable on-disk store under `--data-dir`. `memory` (opt-in, Unix-only like all of `serve`) runs the same engine entirely in RAM: no files, no fsync, NO power-loss or restart durability (a clean stop or crash loses every acked message; a restart revives an EMPTY broker). It refuses to boot without the dedicated `--ephemeral-loss-ack` consent flag and an explicit `--max-total-bytes` above 0 (the RAM bound; the cap meters stored, post-compression bytes), and `--data-dir` must be absent. The startup banner states the ephemeral contract and the materialized-config line says `storage=memory`. Caveat: the dead-letter sink is deliberately uncapped (poison evidence is never shed) and in memory mode it also lives in RAM, outside `--max-total-bytes`, so RAM stays bounded for ack-progressing workloads; pair memory mode with consumers that ack, monitor `ironbus_dlq_records_total`, and tune `--max-deliver`. |
...
attempt 1", the historical behavior, and never blocks startup.
