...
| --- | --- | --- | --- |
| T1 | **Physical device theft** (disk read offline) | At-rest AEAD encryption phase 1 (#780) ships behind the opt-in `encryption` feature (AES-256-GCM / ChaCha20-Poly1305), OFF by default; a default deployment's log and DLQ are still plaintext on disk. | With a key configured, record bodies are ciphertext on disk; the live-broker read-path wiring is follow-on. A key co-located with the stolen disk defeats it (accepted residual risk, see below). |
...
| T4 | **Unauthenticated client produces / consumes / forges** | **Mitigated when auth is configured (#631); opt-in.** On an auth-required broker the `Connect` handshake carries the credential (`parse_connect_auth`), the identity is verified and its scope set pinned to the connection, and every later verb is authorized against that pinned set (a mismatch returns the uniform `AuthError::Violation`, no oracle). On a broker with NO configured auth table (the dev default), `Connect` is unauthenticated and any client that reaches the port has full access. Connection-scoped lease ownership (#175) additionally fences cross-connection acks. | Residual risk: deploying WITHOUT configuring auth, or before TLS (#766) encrypts the credential on a non-loopback bind. Configure an auth table and keep the broker on loopback / a trusted tunnel until TLS lands. |
| T5 | **Stolen bearer token** | **Partially mitigated (#631).** Bearer tokens exist and are scoped, so a leaked token grants only its configured scopes; rotation is additive (add-then-remove, no clock), so a compromised credential is revoked by removing it from the identity's set. | Until TLS (#766) encrypts the wire, a token on a non-loopback plaintext bind can be sniffed in transit; keep the wire on loopback / a trusted tunnel. Time-bounded / auto-expiring credentials remain out of v1 scope. |
| T6 | **Credential brute force by reconnect** | **Mitigated (#633).** A reconnect-and-retry guessing loop is bounded by the per-source failed-auth lockout: after `--auth-failure-lockout` failures from an IP within a window, that IP's new connections are refused for a cooldown (`crates/ironbus-server/src/preauth.rs`, recorded as `ironbus_connections_rejected_total{reason="locked_out"}`). The per-IP connect rate limit further caps how fast one source can even attempt. Both are clock-injected (monotonic, NTP-step-immune) and active on a non-loopback bind. | The lockout is per-source-IP, so a wide spoofed-source flood can still churn the bounded per-IP table; the global half-open cap bounds total concurrent unauthenticated work regardless. Time-bounded / auto-expiring credentials remain out of v1 scope. |
| T7 | **Pre-authentication handshake / connection flood (DoS)** | **Mitigated.** A hard connection cap bounds concurrent connections: the accept loop refuses a new connection once `active >= max_connections` by dropping the stream, and the slot is released even on a panic unwind via a drop guard (`crates/ironbus-server/src/server.rs`, `serve` / `ConnectionSlot`); default 256 (`DEFAULT_MAX_CONNECTIONS`). A slowloris client is bounded by a 30-second read/write timeout (`CONNECTION_TIMEOUT`) that closes a connection making no progress. The frame decoder validates the length prefix against `MAX_FRAME_LEN` (16 MiB + 64 KiB) *before any allocation*, so a hostile length cannot force a large reservation (`decode_frame_with_cap` in `crates/ironbus-proto/src/frame.rs`). The health endpoint bounds its request line at 8 KiB and its per-connection time at a 5-second timeout (`crates/ironbus-server/src/health.rs`). **PER-SOURCE limits now exist too (#633):** a per-IP connect rate limit (token bucket, bounded map with least-recently-seen eviction so limiter memory is O(cap)), a global half-open (accepted-not-yet-authed) cap, and the failed-auth lockout — all O(1)-bounded and checked at accept BEFORE a handler spawns or a handshake byte is read, surfaced on `ironbus_connections_rejected_total{reason}` (`crates/ironbus-server/src/preauth.rs`). | The per-IP limiter table is bounded, so a sufficiently wide spoofed-source-IP flood can churn it (an evicted attacker's bucket resets); the global half-open cap still bounds total concurrent unauthenticated work. Native TLS 1.3 on the wire (#107) is still not wired, so a non-loopback bind is the explicit plaintext opt-in (TLS terminated upstream). With auth configured, a flood of *authenticated* valid produces is governed by the backpressure / token-bucket sheds, not this layer. |
| T8 | **Malicious admin-scope misuse** | **Mitigated when auth is configured (#631).** The three independent scopes (publish / subscribe / admin, no implication) are implemented and pinned at `Connect`; an `admin` verb requires the `admin` scope and is never granted by `publish` / `subscribe`. On a no-auth broker every reachable client has full access (see T4). | With auth configured, the blast radius is bounded to the granted scopes; mutating admin verbs are still being built (#299). |
...
| Per-source-IP connect rate limit (token bucket, bounded map + LRU eviction), global half-open cap, failed-auth lockout — all O(1)-bounded, clock-injected, pre-handler; `rejected_total{reason}` | `PreAuthGuard`, `preauth.rs`; `serve_inner`, `server.rs` (#633) | T6 (credential brute force), T7 (per-source flood) |
...
- **A CRC is not a cryptographic integrity check (T-file).** It detects accidental
  ...
  to the data.
- **Per-source pre-auth limits are bounded, so a wide spoofed-IP flood can churn
  the per-IP table (T6/T7).** The per-IP rate limit + failed-auth lockout now exist
  (#633) and are O(1)-bounded with least-recently-seen eviction; an attacker who
  rotates across more than `MAX_TRACKED_IPS` source IPs can evict and reset another
  source's bucket/lockout, but the GLOBAL connection cap and half-open cap still
  bound total concurrent unauthenticated work regardless. Native wire TLS (#107)
  remains the open transport-encryption gap.
...
## Security controls: implemented vs still-specified

Each control below maps to its tracking issue. Most have since shipped; the remaining unimplemented
controls are the **TLS transport** and **at-rest encryption**.

| Control | Status | Tracking issue |
| --- | --- | --- |
| Connection-scoped authentication; three authorization scopes (publish / subscribe / admin), specified in [AUTHENTICATION.md](AUTHENTICATION.md) | **IMPLEMENTED** (opt-in; pinned at the `Connect` handshake) | #631 |
| Pre-auth DoS defenses (per-source rate limits, half-open caps, failed-auth backoff / lockout) | **IMPLEMENTED** | #633 |
| Secret handling and redaction, the structured security audit-event stream, fail-closed on unsafe secret-file permissions. Specified in [SECRETS.md](SECRETS.md). | **IMPLEMENTED** | #109, #635 |
| TLS 1.3 transport that carries the credential on a non-loopback bind. Specified in [TRANSPORT.md](TRANSPORT.md). | **NOT implemented** — the wire is plaintext today | #766 |
| Optional at-rest AEAD encryption (AES-256-GCM / ChaCha20-Poly1305) and its interaction with checksums and recovery, specified in [AT_REST_ENCRYPTION.md](AT_REST_ENCRYPTION.md) | **Phase 1 implemented** (opt-in `encryption` feature): pure-Rust AEAD core, on-disk format, segment encrypt/decrypt, reason codes, raw-key-file source — OFF/byte-identical by default. Follow-on: live-broker read-path wiring, Argon2id/TEE key sources | #780 |
