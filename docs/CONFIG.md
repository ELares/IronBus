...
| (no file key: flag/env ONLY) | `--commit-gather-us` / `IRONBUS_COMMIT_GATHER_US` | u64 | `200` (`DEFAULT_COMMIT_GATHER_US`) | us | `0..=1000000` | **DEPRECATED AND IGNORED** (#1040; was #454, #472). The durable sync tier now pipelines its fdatasync with a SELF-CLOCKING group commit (the in-flight barrier is the batching window), which strictly dominates the retired wall-clock gather. Still parsed and validated so existing configs keep booting; a non-default value logs a one-line startup deprecation. Config compatibility only — wire/format untouched. |
| (no file key: flag/env ONLY) | `--sync-max-dirty-bytes` / `IRONBUS_SYNC_MAX_DIRTY_BYTES` | size | `16777216` (16 MiB, `DEFAULT_SYNC_MAX_DIRTY_BYTES`) | bytes | `>= 0` (`0` disables the bound) | COLD (an actor-spawn snapshot) (#1040). The pipelined sync tier's dirty-byte admission bound: the most unsynced (appended-but-not-yet-fdatasync'd) record bytes the append actor admits before it THROTTLES a producer until the in-flight barrier completes (never a shed). Bounds the recovery window and page-cache exposure under a slow disk. Enforced only where an ack waits on a covering fsync (durability `sync` on disk, #1026). |
...
| `default_message_ttl_ms` | `--default-message-ttl-ms` / `IRONBUS_DEFAULT_MESSAGE_TTL_MS` | duration | `0` = off | ms | `>= 0` (`0` disables — records never expire on read) | COLD (#549/#710). The per-stream default message TTL: a record older than `producer timestamp + ttl` (wall-clock seam) is expired — skipped on read, never delivered, reclaimed by the segment reap. Combines LOWER-WINS with a per-message `TTL1` headers-prefix TTL. |
| `max_delay_ms` | `--max-delay-ms` / `IRONBUS_MAX_DELAY_MS` | duration | `259200000` (3 days, `DEFAULT_MAX_DELAY_MS`) | ms | `>= 0` (`0` = unbounded) | COLD (#555). The maximum accepted per-message delivery DELAY: a publish whose `DLY1` headers-prefix delay request exceeds it is rejected fail-closed (`ERR_DELAY_TOO_LONG`), never silently clamped — an un-due record pins its group cursors and thereby the retention reap floor, so an unbounded schedule pins retention. The broker resolves an accepted request against ITS wall clock at append into a stored absolute `DUE1` due-instant (clock-skew-safe); release order among due records is log sequence (FIFO-with-delays). |
| `dead_letter_exchange` | `--dead-letter-exchange` / `IRONBUS_DEAD_LETTER_EXCHANGE` | string (comma list) | empty = none | subdir names | 1..=16 unique names of `[A-Za-z0-9._-]` (not `.`/`..`, not a reserved layout name); `dlq` addresses the classic sink | COLD (#551/#710). The dead-letter EXCHANGE: every dead message (max-deliver, TTL-expired with the flag below, `Term` reject) fans out to EVERY listed target, reason-tagged. Empty keeps the fixed `dlq/` sink byte-identical. Named streams route through the same list under `streams/<hex>/`. |
| `dead_letter_expired` | `--dead-letter-expired` / `IRONBUS_DEAD_LETTER_EXPIRED` | bool | `false` | -- | `true`/`false` | COLD (#549/#551/#710). Routes a TTL-expired message to the exchange (reason `ttl-expired`) instead of silent reclamation. Inert without `dead_letter_exchange` (draws a coupled-set warning). |
