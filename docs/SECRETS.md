...
| **Pre-auth rejection** (`preauth_rejection`) | source IP, reason (`rate_limited` / `half_open_cap` / `locked_out`) | A pre-auth DoS defense refused a connection at accept time — the per-source rate limit, the global half-open cap, or the failed-auth lockout (#107, #887). Emitted from the accept loop BEFORE a handler thread or handshake byte, so the audit stream reflects the throttle/lockout decision (not just the `rejected_total{reason}` metric) exactly during a lockout/attack, when no handler-side authn-outcome event can fire. The `reason` is the same bounded tag as the metric. |
