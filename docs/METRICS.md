...
| `ironbus_dead_lettered_total` | A message is dead-lettered: it exceeds `MaxDeliver`, or (with a dead-letter exchange configured) a TTL-expired/rejected message is routed to it. The message is committed past and moved to the dead-letter sink, recording the reason. | **Dead-letter**: a poison/expired/rejected message is removed from the main flow into the (default or configured) dead-letter sink rather than looping forever or vanishing. |
| `ironbus_expired_total` | A message's per-message/per-stream TTL has passed when a consumer reaches it, so it is skipped on read (never delivered) and committed past — its bytes reclaimed by the segment retention reap. Counts only the **reclaimed** (not dead-lettered) expiries; a TTL expiry routed to a dead-letter exchange increments `dead_lettered` instead. | **Expire-and-reclaim**: a perishable message ages out without delivery. Bounded (no per-message timer; piggybacks the reap) and **accounted** (every expiry is in exactly one of `expired` or `dead_lettered`, never a silent drop). |
...
| `ironbus_producer_out_of_order_total` | An **idempotent-producer** `produce` carried a monotonic `sequence` that skipped past the next-expected (`seq > last-accepted + 1`, a gap), so the broker **rejected** it (the Kafka `OutOfOrderSequence` rule) and appended nothing (V2-M8, #638). | **Out-of-order rejection**: a silent reorder is refused so a later retry of a skipped seq can never double-append, keeping the effectively-once guarantee intact. A non-zero rate means a producer's sequence stream has a genuine gap (a lost in-flight publish, or a client bug). A sequenced retry *hit* shares `ironbus_dedup_hits_total` (the same observable as a `msg_id` dedup hit); the **durable** per-producer high-water that survives a restart + a long offline gap carries no separate gauge. |
...
| `ironbus_checkpoint_damaged_total{artifact}` | A dual-slot checkpoint opened with **both** slots carrying a nonzero sequence yet **both** failing their CRC — impossible from any crash (each write touches one slot, so a crash tears at most one), so provable **external** damage (bit rot / a lost extent), NOT the never-written `None` the pre-#1142 open collapsed it into (#1142). One increment per damaged open, by on-disk `artifact` (`cursor`, `attempts`, `counters`, `producer_seq`, `metadata_snapshot`, `shared_wal_reap`, `bindings`, `geo_cursor`, `cold_manifest`). The broker warns and recovers as **empty** — the load-bearing checkpoints (`bindings`, `shared_wal_reap`, `cold_manifest`) keep their own downstream fail-closed guards (a loud NoStream; the physical-earliest cross-check; the manifest fail-closes an undecodable payload), so empty-recovery is never a silent correctness loss. | **External-damage detection, observable + non-fatal**: turns a formerly SILENT empty-recovery into a LOUD, alertable data-availability event without making it a broker-won't-start regression. NATS has no analogue. |
| `ironbus_cold_offload_errors_total{reason}` | A best-effort tiered-storage (#643) offload FAILED on the retention tick, by `reason` (`cold_store` = a `put`/`get` transport error or no backend configured; `other` = the manifest at its slot cap, a local IO error, etc.). Offload runs AFTER a produce's commit, so a cold-store outage must never fail the produce: the broker `warn!`s, increments this counter, and CONTINUES — the segment stays local and the next tick retries. | **Degraded-tiering detection, non-fatal**: a non-zero value means cold data is not being tiered out (a backend outage or a full manifest), without ever taking writes down. NATS has no tiered-storage analogue. |

`ironbus_recovery_runs_total`, `ironbus_corruption_repairs_total`,
`ironbus_checkpoint_damaged_total`, and `ironbus_cold_offload_errors_total` are **labeled**
