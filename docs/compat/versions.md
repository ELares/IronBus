...
| `codec` id | `none` = `0`, `lz4` = `1` (implemented); `zstd` = `2` (reserved, opt-in, not on the default build) | `ironbus_core::compress::Codec` (`CODEC_ID_NONE`/`CODEC_ID_LZ4`/`CODEC_ID_ZSTD_RESERVED`), carried in the compressed-payload descriptor behind `RecordFlags::COMPRESSED`; see [CONTRACTS.md](../CONTRACTS.md#compressed-payload-descriptor-when-compressed-is-set), [DICTIONARY_LIFECYCLE.md](../DICTIONARY_LIFECYCLE.md), and [ADR 0003](../adr/0003-default-compression-lz4-zstd-opt-in.md) | Append-only. New codec ids are appended; `lz4` (#387) is the pure-Rust default, `zstd` stays a deferred opt-in feature. | #12, #387 |
| `dict_id` | `0` = no-dictionary sentinel (the only value the default `lz4` build writes); the `u32` field is carried per record | `u32` in the compressed-payload descriptor (`ironbus_core::compress`); content-addressed per [DICTIONARY_LIFECYCLE.md](../DICTIONARY_LIFECYCLE.md). The descriptor field + the `UnresolvedDictId` POISON path are implemented (#357); ZDICT training is deferred with `zstd` | Append-only (content-addressed; reuse is structurally impossible). | #12, #78, #357 |
