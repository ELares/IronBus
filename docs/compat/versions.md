...
| Data-dir layout version | `1` | `ironbus_storage::layout::LAYOUT_VERSION` (the `layout.meta` marker, CRC32C two-slot checkpoint) | Breaking. Versions the on-disk DIRECTORY structure (where streams/cursors/DLQ live), SEPARATELY from `FORMAT_VERSION` (which versions the frame ENCODING). v1 = today's root-log + `dlq/` layout and the `streams/` subtree (M2-I2, now live): each named stream is `streams/<hex(stream)>/` holding its own log AND (since #681) its per-group consumer cursor checkpoints `cursor-<hex(group)>.ckpt` — the default stream's `cursor.ckpt` snapshot format, verbatim, just rooted in the stream's subdir. Adding those cursor files does NOT bump the layout version: they are additive regular files a v1 reader tolerates (a pre-#681 broker simply ignores a named stream's cursor file and resumes that consumer at offset 0 — at-least-once safe, redelivering only already-acked records; a #681+ broker resumes at the committed offset). A v1 reader refuses any higher value; an absent marker auto-upgrades to v1 (an existing single-log dir is byte-for-byte v1); a torn/corrupt marker recovers as v1 and never bricks the dir. | #562, #500, #681 |
...
| `RecordFlags` bits | `KNOWN` = `0b1111` (`COMPRESSED`=bit0, `HAS_KEY`=bit1, `HAS_XXH3`=bit2, `HAS_SUBJECT`=bit3) | `ironbus_core::types::RecordFlags` (`COMPRESSED`/`HAS_KEY`/`HAS_XXH3`/`HAS_SUBJECT`/`KNOWN`) | Append-only. A new flag claims a higher bit; unknown bits are preserved, never interpreted, within a known version. `HAS_SUBJECT` (#594) is additive: when set, an optional subject field (`subject_len: u16`, subject bytes, `subject_crc: u32` over the two — sizes `RECORD_SUBJECT_LEN_PREFIX` / `RECORD_SUBJECT_CRC_LEN`) sits immediately after the 36-byte header and before the body, counted in `total_len`; a record without the bit is byte-for-byte the pre-subject layout, and the body CRC32C/xxh3 range is unchanged (the subject has its own CRC). `FORMAT_VERSION` stays `1`. **Downgrade caveat (#594):** because the bit is additive within `FORMAT_VERSION` 1, a pre-#594 (or downgraded) reader does NOT know the subject field is present; it reads the flags byte (the header CRC still passes — the field is a preserved unknown bit) but computes a `total_len` that omits the subject field, so it mis-frames the body and the record fails `body_crc` as `BadBodyCrc`. On recovery that is treated as a corrupt/torn frame, which **truncates the log at the first `HAS_SUBJECT` record — a data-loss event ON DOWNGRADE**. Forward (upgrade) is always safe; cross-feature DOWNGRADE past #594 (running an older binary against a log that already stored a subject record) is UNSUPPORTED. | #5, #12, #594 |
...
```text
format-layout-sha256: 5dd5d08a18dc2b9da03acc46463b3993cf272ca489328d3f91979ba05c25635e
```
