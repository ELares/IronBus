...
| `FrameType` tag set | `1..=19`, contiguous (`Connect`=1 .. `CumulativeAck`=19) | `ironbus_proto::frame::FrameType::as_u8`/`from_u8`; frozen by `type_tags_have_their_exact_frozen_wire_values` | Append-only. The next frame takes tag 20; no existing tag's meaning ever changes. | #11 |
