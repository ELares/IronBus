...
| Cold-segment manifest format (tiered storage) | `1` | `ironbus_storage::cold` `COLD_MANIFEST_VERSION` = `1`; the `IBCM` magic + fixed 53-byte entries, in the `cold-manifest.ckpt` dual-slot CRC32C checkpoint; pinned by `ironbus_storage::cold::tests::manifest_corrupt_payload_fails_closed` and the `cold_tiering.rs` recovery tests | Breaking. Records which SEALED segments are offloaded to the object store (REMOTE) so recovery treats an offloaded segment as PRESENT, not a torn gap. A reader REFUSES a manifest whose version byte is higher than known, and FAIL-CLOSES (`StorageError::ColdManifestCorrupt`) on a CRC-valid-but-undecodable payload — an acked REMOTE transition is load-bearing (never silently dropped). Additive/lazy on disk (no `LAYOUT_VERSION` bump). | #643 |
| `FrameType` tag set | `1..=52`, contiguous (`Connect`=1 .. `GapMarker`=21, `ProduceConfirm`=22 .. `TxnListen`=49, `RaftAuth`=50, `DataPlaneAuth`=51, `PauseGroup`=52; the client-plane, peer-plane, txn, interim peer-auth, and consumer-control verbs — see the full table in [CONTRACTS.md](../CONTRACTS.md)) | `ironbus_proto::frame::FrameType::as_u8`/`from_u8`; frozen by `type_tags_have_their_exact_frozen_wire_values`; the tag map is HASH-PINNED by the `frame-tags-sha256` sentinel below (the same gate that pins the storage layout), so a new tag cannot land without updating this row | Append-only. The next frame takes tag 53; no existing tag's meaning ever changes. (`PauseGroup`=52 is the consumer pause/resume delivery-gate verb, #771 V2-M1.) (`RaftAuth`=50 is the interim HMAC peer-auth envelope for the raft metadata wire, #1067 Inc 2; `DataPlaneAuth`=51 is the same interim HMAC envelope for the cluster data-plane wire, #1067 Inc 3. This row previously rotted at `1..=21` while the code shipped 49 — the hash pin is the forcing function that prevents a recurrence.) | #11, #33, #346, #588, #585, #640, #1067 |
...
| `SegmentHeader.flags` `SEGMENT_FLAG_COMPACTED` bit | bit 0 (`0x0001`) | `ironbus_core::format::SEGMENT_FLAG_COMPACTED`; pinned by `frozen_compaction_v2_values_and_offsets` | Append-only segment-flag bit. Distinct from the at-rest encryption bit (bit 1); a compacted segment sets it and stamps `version = 2`. A v1 reader treats `flags` as preserved-but-not-interpreted; the paired `version = 2` is what makes it fail closed. | #337, #18 |
| `SegmentHeader.flags` `SEGMENT_FLAG_ENCRYPTED` bit + `aead_suite`/`key_id` | bit 1 (`0x0002`); `aead_suite` u8 at header offset 44, `key_id` u64 at `[45, 53)` (the previously-reserved `[44, 60)` region, inside the frozen `header_crc` scope `[0, 60)`) | `ironbus_core::format::{SEGMENT_FLAG_ENCRYPTED, AEAD_TAG_LEN, AEAD_SUITE_NONE, AEAD_SUITE_AES_256_GCM, AEAD_SUITE_CHACHA20_POLY1305, segment_header_offsets::AEAD_SUITE, segment_header_offsets::KEY_ID}` and `ironbus_core::segment::SegmentHeader::{encode_encrypted, aead_params, is_encrypted}`; pinned by `frozen_at_rest_encryption_values`, `segment_header_and_footer_offsets`, and `an_encrypted_header_records_suite_and_key_id_and_round_trips` | Append-only segment-flag bit (#780). Set on an AEAD-encrypted segment; DISTINCT from `SEGMENT_FLAG_COMPACTED` (a segment may be both). Encryption does NOT bump `version` — it reuses reserved bytes, so an encrypted (non-compacted) segment stays `version = 1`, byte-for-byte the pre-encryption header when the flag is clear (the reserved region is all-zero). `aead_suite` REFUSES an unknown value on read (like `checksum_algo`); the 16-byte record tag is what fail-closes a pre-encryption reader. | #780, #18 |
...
| `ReasonCode` vocabulary | codes `1..=9` (`TornTail`=1 .. `ScrubberSuspect`=6, `UnresolvedDictId`=7, `UnknownKeyId`=8, `AeadTagMismatch`=9) | `ironbus_storage::loss::ReasonCode::code`; frozen by `golden_reason_code_vocabulary_is_frozen` and `reason_codes_are_stable_and_distinct` | Append-only. A new reason gets a new code/name/label and does NOT bump `schema_version`. `UnknownKeyId` (#780) = a segment's `key_id` matches no loaded at-rest key; `AeadTagMismatch` (#780) = the CRC over the ciphertext passed but the AEAD tag failed under the named key (wrong/rotated key or forgery). Both are DISTINCT from `CorruptRecordBody` so a key problem never masquerades as bit-rot; both route through the #8 bounded-and-reported loss path. | #11, #59, #357, #780 |
| `RecordFlags` bits | `KNOWN` = `0b11_1111` (`COMPRESSED`=bit0, `HAS_KEY`=bit1, `HAS_XXH3`=bit2, `HAS_SUBJECT`=bit3, `HAS_STREAM_TAG`=bit4, `ENCRYPTED`=bit5) | `ironbus_core::types::RecordFlags` (`COMPRESSED`/`HAS_KEY`/`HAS_XXH3`/`HAS_SUBJECT`/`HAS_STREAM_TAG`/`ENCRYPTED`/`KNOWN`) | Append-only. A new flag claims a higher bit; unknown bits are preserved, never interpreted, within a known version. `HAS_SUBJECT` (#594) is additive: when set, an optional subject field (`subject_len: u16`, subject bytes, `subject_crc: u32` over the two — sizes `RECORD_SUBJECT_LEN_PREFIX` / `RECORD_SUBJECT_CRC_LEN`) sits immediately after the 36-byte header and before the body, counted in `total_len`; a record without the bit is byte-for-byte the pre-subject layout, and the body CRC32C/xxh3 range is unchanged (the subject has its own CRC). `HAS_STREAM_TAG` (#597, the shared-WAL fallback) is likewise additive and STRUCTURALLY IDENTICAL to the subject field — an optional `stream_tag_len: u16`, tag bytes, `stream_tag_crc: u32` (sizes `RECORD_STREAM_TAG_LEN_PREFIX` / `RECORD_STREAM_TAG_CRC_LEN`) at the SAME fixed post-header offset — and is MUTUALLY EXCLUSIVE with `HAS_SUBJECT` (a frame that sets both is rejected `BadLength`; the two share the slot). It stores the stream a record belongs to when many streams share ONE tagged commit log (`ironbus_storage::shared_wal::SharedWal`); a record without the bit is byte-for-byte the pre-tag layout, and the body checksum range is unchanged (the tag has its own CRC). `ENCRYPTED` (#780, at-rest AEAD) is additive: when set, the on-disk body is the AEAD ciphertext (same length as the plaintext body) followed by the fixed 16-byte tag (`AEAD_TAG_LEN`), counted in `total_len` like the xxh3 field; the `key_len`/`hdr_len`/`payload_len` header fields describe the PLAINTEXT lengths, the tag is the only size delta, and the #5 CRC32C (and xxh3 for a large body) cover the CIPHERTEXT + tag so corruption is detected WITHOUT the key. An encrypted record carries no subject/stream-tag slot (v1 uniformity), and the plaintext `codec::decode` REFUSES it (`DecodeError::Encrypted`) so ciphertext is never split as plaintext — the encrypted read path uses `codec::decode_encrypted` then AEAD-decrypts. `FORMAT_VERSION` stays `1`. **Downgrade caveat (#594, #597, #780):** because these bits are additive within `FORMAT_VERSION` 1, a reader that predates them does NOT know the optional field is present; it reads the flags byte (the header CRC still passes — the field is a preserved unknown bit) but computes a `total_len` that omits the field, so it mis-frames the body and the record fails `body_crc` as `BadBodyCrc` (for `ENCRYPTED`, the 16-byte tag inflates the body beyond the declared plaintext lengths, so the pre-encryption reader rejects the frame as `BadLength`/`BadBodyCrc` rather than ever reading ciphertext as plaintext). On recovery that is treated as a corrupt/torn frame, which **truncates the log at the first `HAS_SUBJECT`/`HAS_STREAM_TAG`/`ENCRYPTED` record — a data-loss event ON DOWNGRADE**. Forward (upgrade) is always safe; cross-feature DOWNGRADE past #594/#597/#780 (running an older binary against a log that already stored a subject, stream-tag, or encrypted record) is UNSUPPORTED. | #5, #12, #594, #597, #780 |
...
| Cold-segment manifest version (`cold-manifest.ckpt`, tiered storage) | REFUSE a higher version, and FAIL-CLOSED on a CRC-valid-but-undecodable payload (an acked REMOTE transition is load-bearing, never silently emptied); a TORN slot falls back to the prior durable manifest; an ABSENT manifest means "nothing offloaded" (byte-identical default-OFF) | no (breaking) | #643 | `StorageError::ColdManifestCorrupt`; `ironbus_storage::cold::ColdManifest::open`, spliced at the end of `Log::open` |
...
```text
format-layout-sha256: 139e94c45df6a526ae401a92c7850864e59985e49d015d8d9f42c8d880b66e15
```
...
```text
frame-tags-sha256: 57dbfef1cf7a8079984103f26851357042500013fb58bd1f9974106fa95af3df
```

The frame-tag digest was last re-pinned for the APPEND-ONLY `PauseGroup` tag (52, #771 V2-M1,
the consumer pause/resume delivery gate). Tags 1..=51 are byte-for-byte unchanged.
