...
| F15 | **Unbounded per-consumer occupancy**: one consumer fetches without acking and pins unbounded in-flight messages, blowing the RAM ceiling and starving peers. | #9, #10, #65 | High | `ironbus_in_flight` / `ironbus_group_in_flight{group}`; the per-connection credit ceiling caps a stuck consumer to its own slots. | Two composed per-connection budgets: the message-count credit (`--consumer-credit`) auto-tunes from a 64 floor toward a 2048 ceiling (`DEFAULT_CREDIT_CEILING`, the default; it halves under backpressure) and the byte budget (`--consumer-credit-bytes`, default 8 MiB) is the FIRM RAM bound the count grows strictly UNDER, so the higher count ceiling never blows RAM; effective credit = `min(...)` with a one-message floor; derived from the connection-scoped `leased` set so accounting cannot drift (#65, #275, #552). See RR-01. | Session credit-cap / restore-on-ack/nack/term / isolation / redelivery-accounting tests; the end-to-end over-a-real-server credit test; CLI flag-validation tests. |
| F16 | **Connection flood / slowloris / oversized frame**: an unauthenticated peer opens unbounded connections, holds them open without progress, or declares a huge frame length to force a giant allocation. | #16, #18, #10 | High | Connections past the cap are dropped; a slowloris hits the 30 s `CONNECTION_TIMEOUT`; an oversized length is the typed `FrameTooLarge` BEFORE allocation. | A connection cap (`--max-connections`, default 256, slot released on panic), a 30 s read/write deadline, and `MAX_FRAME_LEN` (16 MiB + 64 KiB) validated before any body allocation (#105). PER-SOURCE limits now exist too (#633): a per-IP connect rate limit, a global half-open cap, and a failed-auth lockout, all O(1)-bounded and checked BEFORE a handler spawns (see F25). See RR-08 and THREAT_MODEL.md (T7). | `frame.rs` `FrameTooLarge` / `EmptyFrame` decode tests; server connection-cap / `ConnectionSlot` drop-guard tests; the health-endpoint 8 KiB / 5 s bound tests. |
...
| F19 | **No auth / no TLS on a non-loopback bind** exposes produce/consume/ack and `/metrics` to any reachable peer, and plaintext on the wire to any observer. | #16, #18, #106, #107, #629, #631 | High | A non-loopback bind is REFUSED pre-listen unless it carries an auth identity AND the explicit plaintext opt-in; the uniform Authorization Violation on every authn/authz failure. The default bind stays `127.0.0.1:7777`. | Connection-scoped auth + three-scope authz (#106/#631) and the fail-closed non-loopback bind invariant (#629) are BUILT: a non-loopback bind without an auth identity is rejected at startup; every verb is scope-gated; the loopback dev path is byte-for-byte unchanged. **Remaining gap (OPEN):** native TLS 1.3 on the wire (#107) is not yet wired, so a non-loopback bind today is the explicit `--insecure-plaintext-wire` opt-in (TLS terminated upstream). See RR-16 / RR-17, THREAT_MODEL.md (T2-T4), AUTHENTICATION.md, TRANSPORT.md. | `session.rs` scope-gate tests (`a_produce_without_the_publish_scope_is_refused`, `admin_does_not_imply_publish_or_subscribe`, `a_no_auth_broker_is_byte_for_byte_unchanged_zero_config_dev`); the CLI bind-guard refusal tests; `load_auth_config` round-trip. TLS (#107) is still uncovered. |
...
| F25 | **Pre-auth credential-guessing / connection flood from one source**: an UNAUTHENTICATED attacker hammers `Connect` to grind passwords (forcing the deliberately-expensive Argon2id verify), or floods half-open connections to exhaust handler slots, before ever authenticating. | #18, #633 | High | `ironbus_connections_rejected_total{reason}` with `reason` one of `rate_limited` / `half_open_cap` / `locked_out` / `auth_failed` (a bounded, low-cardinality counter). | The pre-auth `DoS` defenses (#633), checked at accept BEFORE a handler spawns or a handshake byte is read: a per-source-IP connect token bucket (bounded map with least-recently-seen eviction, so limiter memory is O(cap), never O(distinct IPs)), a global half-open (accepted-not-yet-authed) cap via an RAII slot, and a failed-auth lockout (the `auth_failure_lockout` threshold over a window, then a cooldown). All clock-injected (monotonic, NTP-step-immune) and ON for a non-loopback bind, inert on loopback. **Residual:** a sufficiently wide spoofed-source-IP flood can churn the per-IP table; the global half-open cap still bounds total concurrent unauthenticated work. See THREAT_MODEL.md (T7), AUTHENTICATION.md. | `preauth.rs` unit tests (rate limit triggers + refills, per-IP isolation, half-open cap + slot free, lockout + cooldown lapse, window expiry, bounded-map eviction, legit-client-never-rejected); `server.rs` end-to-end `preauth_rate_limit_rejects_a_second_rapid_connection_and_emits_the_metric`; `connz.rs` `rejected_total{reason}` accounting. |
